...
def training():
    # Load the data
    train = pd.read_csv('kitchen-images/data/train.csv')
    test = pd.read_csv('kitchen-images/data/test.csv')
    ...
    logging.info(f'Total number of images in the images folder: {len(os.listdir("./kitchen-images/images"))}')
    ...
    train_generator = train_gen.flow_from_dataframe(
        dataframe=train,
        directory='./kitchen-images/images/',
        x_col='Id',
        y_col='label',
        target_size=(299, 299),
        batch_size=32,
    ...
    history = model.fit(
        ...
        callbacks=[
            ModelCheckpoint('./kitchen-images/models/model.h', save_best_only=True),
            EarlyStopping(patience=3)
        ]
    )

    logging.info("Training Completed, model saved to ./models/model.h")
...
def predict_single(model, image_url, web=True):
    ...
    with open('./kitchen-images/models/class_indices.json', 'r') as f:
